...
|---|---|---|
| `q00_schema.sql` | schema of `works` (INFORMATION_SCHEMA, free), row counts and bytes (`__TABLES__`, free), the xpac/core split, a 5-row preview with SQL (to show that LIMIT does not reduce bytes), the "SELECT *" sin | `subugoe-collaborative.openalex_walden.works` |
...
4. Suggested order: `q00a`, `q01`, `q02` (its step 0 creates the `daisy` dataset, then upload `ce_topics.csv`, then the query), `q03` (block 0 first, then blocks 1 to 5, then block 7 for the export). `q00` and `q04` are reference material.
